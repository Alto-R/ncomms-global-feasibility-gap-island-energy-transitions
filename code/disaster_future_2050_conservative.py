...
# PEM and FC follow the same Conservative/Moderate uplift ratio as ESS for the 2050 sensitivity case.
investment_cost = {'WT': 1392,'PV': 1377, 'WEC': 6000,'AC':150, 'EB': 250,'CHP': 1300,'PEM': 1247.792992, 'FC': 2669.075918, 'LNG': 700,'LNGV': 500, 'ESS': 980.8854, 
...
# 设备失效状态聚类
 # --- 参数设置 ---
num_scenarios_to_generate = 10000 # 生成10000个原始场景用于分析
...
cost_df.to_csv(f'{output_dir}/{island_lat}_{island_lon}_best_cost.csv', index=False)
capacity_df.to_csv(f'{output_dir}/{island_lat}_{island_lon}_capacity.csv', index=False)
results.to_csv(f'{output_dir}/{island_lat}_{island_lon}_results.csv', index=False)
