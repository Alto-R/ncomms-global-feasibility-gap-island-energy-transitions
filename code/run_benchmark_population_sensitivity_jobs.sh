...
# master0/island/run_benchmark_population_sensitivity_jobs.sh
# master0/island/result/benchmark_population_sensitivity/benchmark_population_sensitivity_run_matrix.csv
# master0/island/demand/get1/...
# master0/island/disaster_2020.py
SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT="$SCRIPT_DIR"
RUN_MATRIX_FILE="$REPO_ROOT/result/benchmark_population_sensitivity/benchmark_population_sensitivity_run_matrix.csv"
LOG_DIR="$REPO_ROOT/logs/benchmark_population_sensitivity"

PYTHON_BIN="${PYTHON_BIN:-python3}"
SOURCE_DATA_DIR="${SOURCE_DATA_DIR:-$REPO_ROOT/demand/get1}"
SOURCE_POP="${SOURCE_POP:-500}"
...
usage() {
    cat <<'EOF'
Usage:
  bash code/run_benchmark_population_sensitivity_jobs.sh [all|output_0|output_2050] [options]
...
Options:
  ...
  --source-data-dir DIR      Benchmark demand/resource directory. Default: demand/get1
  --source-pop N             Benchmark source population. Default: 500
  -h, --help                 Show this help message.

Examples:
  bash run_benchmark_population_sensitivity_jobs.sh
  bash run_benchmark_population_sensitivity_jobs.sh output_0 --force --max-concurrent 24
  bash run_benchmark_population_sensitivity_jobs.sh output_2050 --limit 20
  bash run_benchmark_population_sensitivity_jobs.sh all --python-bin /path/to/python3
EOF
}
...
ensure_run_matrix() {
    ...
    echo "Run matrix not found. Building benchmark sensitivity sample and run matrix..." | tee -a "$MAIN_LOG"
    (
        cd "$REPO_ROOT"
        "$PYTHON_BIN" build_benchmark_population_sensitivity_sample.py
    ) >> "$MAIN_LOG" 2>&1
}
...
run_scenario_batches() {
    ...
    echo "Source data dir: $SOURCE_DATA_DIR" >> "$MAIN_LOG"
    echo "Source population: $SOURCE_POP" >> "$MAIN_LOG"
    ...
    ensure_run_matrix
    if [ ! -f "$RUN_MATRIX_FILE" ]; then
        echo "Run matrix still missing: $RUN_MATRIX_FILE" | tee -a "$MAIN_LOG"
        return 1
    fi
...
case "$SCENARIO_TARGET" in
    all)
        run_scenario_batches "output_0" "disaster_free" || exit 1
        run_scenario_batches "output_2050" "disaster_2050" || exit 1
        ;;
    output_0)
        run_scenario_batches "output_0" "disaster_free" || exit 1
        ;;
